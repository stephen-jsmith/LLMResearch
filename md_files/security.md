...
The Security Kernel (SK) microservice provides role-based authorization
...
SK authorization is based on an extended version of the Apache
[Shiro](https://shiro.apache.org/) authorization model, which defines
both [roles](#roles) and [permissions](#permissions).

## Roles

In a tenant, each role has a unique name and is assigned an owner. SK
...
Each tenant has at least one tenant administrator, which is nothing more
...
One feature that distinguishes SK\'s implementation of role-based
...
In addition to checking whether a user has been granted a certain role,
...
SK implements the full Shiro permission model and extends it to
...
SK uses HashiCorp [Vault](https://www.hashicorp.com/products/vault) as
